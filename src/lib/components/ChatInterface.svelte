<script lang="ts">
	...
	import { type ChatMessage as RpcChatMessage } from '$lib/chatIntefaces'; // Keep for ChatMessage structure
	...
	function mapServerToClientMessage(
		serverMsg: RpcChatMessage,
		convId: string,
		messageTimestamp: Date,
		internalId?: string
	): Omit<ClientMessage, 'id'> {
		return {
	...
	$: if ($conversationId && socket?.connected) {
		// Request history if conversationId changes and socket is connected
	...
	onMount(() => {
		...
		socket.on('chatChunk', (data: { conversationId: string; type: 'chunk'; data: string }) => {
			...
			if (data.conversationId === $conversationId && currentAiMessageId !== null) {
				...
				dxDb.messages.update(currentAiMessageId, {
					text: currentAiAccumulatedText + '...',
					timestamp: new Date()
				});
			}
		});
		...
		socket.on('streamEnd', (data: { conversationId: string; type: 'end'; data: string }) => {
			console.log(
				'[ChatInterface] Received streamEnd event. Data:',
				data,
				'Current AI Msg ID:',
				currentAiMessageId,
				'Current Conv ID:',
		...
		socket.on('streamError', (data: { conversationId: string; type: 'error'; data: string }) => {
			if (data.conversationId === $conversationId && currentAiMessageId !== null) {
				...
				dxDb.messages.update(currentAiMessageId, {
					text: `Server-side error: ${data.data}`,
					status: 'error',
					role: 'system'
				});
				currentAiMessageId = null;
	...
	async function clearClientDB() {
		...
		if (!socket || !socket.connected || !currentConvId) {
			...
			try {
				...
				console.log(
					'Entire Client DB cleared successfully (remote not cleared as socket was not ready or no convId).'
				);
...
<div class="chat-interface-container grid h-full grid-rows-[auto_auto_1fr_auto] gap-2 p-2">
	<button on:click={clearClientDB} class="btn btn-warning btn-xs self-start">Clear Client DB</button
	>
	<div class="status-bar text-muted-foreground text-xs">
		Status: {$connectionStatus} | Conv ID: {$conversationId
	...
	<div class="messages-area space-y-2 overflow-y-auto">
		{#if $conversationId && messages$ === undefined}
			...
			<p class="text-muted-foreground text-center text-sm">Initializing message stream...</p>
		...
		{:else if actualMessages && actualMessages.length > 0}
			{#each actualMessages as message (message.internalId)}
				<div
					class="message rounded-md p-2 text-sm {message.role === 'user'
						? 'bg-primary text-primary-foreground ml-auto'
						: 'bg-muted mr-auto'} {message.status === 'error' ? 'border-destructive border' : ''}"
					style="max-width: 80%; word-wrap: break-word;"
				>
					<strong class="block font-semibold capitalize">{message.role}</strong>
					<div class="whitespace-pre-wrap">{@html message.text}</div>
					{#if message.status === 'pending'}<span class="text-muted-foreground/70 text-xs">
							(Sending...)</span
						>{/if}
					{#if message.status === 'error' && message.role !== 'system'}<span
							class="text-destructive/80 text-xs"
						>
							(Failed)</span
						>{/if}
				</div>
		...
		{:else if !$conversationId}
			<p class="text-muted-foreground text-center text-sm">No conversation selected.</p>
		{:else}
			<!-- actualMessages is an empty array or messages$ is undefined without a conversationId -->
			<p class="text-muted-foreground text-center text-sm">No messages yet. Send one to start!</p>
		{/if}
	</div>
	<div class="input-area flex gap-2">
		<textarea
			...
			class="textarea textarea-bordered textarea-sm flex-grow resize-none overflow-hidden"
			...
			disabled={!socket?.connected ||
				($connectionStatus !== 'Connected' && $connectionStatus !== 'Connected (History Loaded)') ||
				!$conversationId}
		></textarea>
		<button
